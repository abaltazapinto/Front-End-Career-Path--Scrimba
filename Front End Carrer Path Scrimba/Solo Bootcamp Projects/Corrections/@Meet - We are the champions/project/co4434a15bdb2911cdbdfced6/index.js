...
const appSettings = {
    databaseURL: "https://endorsementsapp-e8b8c-default-rtdb.europe-west1.firebasedatabase.app"
}
...
addBtn.addEventListener('click', function () {
    ...
    push(endorsementsInDB, { text: inputValue, from: fromValue, to: toValue, likes: 0 });
    clearInputFieldEl();
});
...
function appendToEndorsementListEl(inputValue) {
    ...
    let itemValue = itemData.text.charAt(0).toUpperCase() + itemData.text.slice(1).toLowerCase();
    let fromValue = itemData.from.charAt(0).toUpperCase() + itemData.from.slice(1).toLowerCase();
    let toValue = itemData.to.charAt(0).toUpperCase() + itemData.to.slice(1).toLowerCase();
